...
void intersect_rt(double X1, double Y1, double Z1, double a1, double b1,
                  double c1, double X2, double Y2, double Z2, double a2,
                  double b2, double c2, double *X, double *Y, double *Z)
...
/* only valid, if Z1 = Z2 = 0 , which is the case after ray tracing */
{
  ...
  if (a1 / c1 == a2 / c2 &&
      b1 / c1 == b2 / c2) { // if parallel, return high values
...
#ifdef EVER_CALLED // Unused function, ad holten 12-2012
void intersect_rt_3(double X1, double Y1, double Z1, double a1, double b1,
                    double c1, double X2, double Y2, double Z2, double a2,
                    double b2, double c2, double X3, double Y3, double Z3,
...
/* intersection, given three points with direction cosines */
{
  ...
  switch (n_max) {
  case 0:
    *Z = (2 * X3 - X1 - X2) / ((a1 / c1) + (a2 / c2) - (2 * a3 / c3));
    break;
  case 1:
    *Z = (2 * X2 - X1 - X3) / ((a1 / c1) + (a3 / c3) - (2 * a2 / c2));
    break;
  case 2:
    *Z = (2 * X1 - X2 - X3) / ((a2 / c2) + (a3 / c3) - (2 * a1 / c1));
    break;
  case 3:
    *Z = (2 * Y3 - Y1 - Y2) / ((b1 / c1) + (b2 / c2) - (2 * b3 / c3));
    break;
  case 4:
    *Z = (2 * Y2 - Y1 - Y3) / ((b1 / c1) + (b3 / c3) - (2 * b2 / c2));
    break;
  case 5:
    *Z = (2 * Y1 - Y2 - Y3) / ((b2 / c2) + (b3 / c3) - (2 * b1 / c1));
    break;
  }

  *X = (X1 + (*Z) * (a1 / c1) + X2 + (*Z) * (a2 / c2) + X3 + (*Z) * (a3 / c3)) /
       3;
  *Y = (Y1 + (*Z) * (b1 / c1) + Y2 + (*Z) * (b2 / c2) + Y3 + (*Z) * (b3 / c3)) /
       3;
}
#endif

#ifdef EVER_CALLED // Unused function, ad holten 12-2012
void intersect_rt_3m(double X1, double Y1, double Z1, double a1, double b1,
                     double c1, double X2, double Y2, double Z2, double a2,
                     double b2, double c2, double X3, double Y3, double Z3,
                     double a3, double b3, double c3, double *X, double *Y,
                     double *Z)
/* intersection, given three points with direction cosines */
{
  ...
  switch (n_max1) {
  case 0:
    Za = (2 * X3 - X1 - X2) / ((a1 / c1) + (a2 / c2) - (2 * a3 / c3));
    break;
  case 1:
    Za = (2 * X2 - X1 - X3) / ((a1 / c1) + (a3 / c3) - (2 * a2 / c2));
    break;
  case 2:
    Za = (2 * X1 - X2 - X3) / ((a2 / c2) + (a3 / c3) - (2 * a1 / c1));
    break;
  case 3:
    Za = (2 * Y3 - Y1 - Y2) / ((b1 / c1) + (b2 / c2) - (2 * b3 / c3));
    break;
  case 4:
    Za = (2 * Y2 - Y1 - Y3) / ((b1 / c1) + (b3 / c3) - (2 * b2 / c2));
    break;
  case 5:
    Za = (2 * Y1 - Y2 - Y3) / ((b2 / c2) + (b3 / c3) - (2 * b1 / c1));
    break;
  }
  switch (n_max2) {
  case 0:
    Zb = (2 * X3 - X1 - X2) / ((a1 / c1) + (a2 / c2) - (2 * a3 / c3));
    break;
  case 1:
    Zb = (2 * X2 - X1 - X3) / ((a1 / c1) + (a3 / c3) - (2 * a2 / c2));
    break;
  case 2:
    Zb = (2 * X1 - X2 - X3) / ((a2 / c2) + (a3 / c3) - (2 * a1 / c1));
    break;
  case 3:
    Zb = (2 * Y3 - Y1 - Y2) / ((b1 / c1) + (b2 / c2) - (2 * b3 / c3));
    break;
  case 4:
    Zb = (2 * Y2 - Y1 - Y3) / ((b1 / c1) + (b3 / c3) - (2 * b2 / c2));
    break;
  case 5:
    Zb = (2 * Y1 - Y2 - Y3) / ((b2 / c2) + (b3 / c3) - (2 * b1 / c1));
    break;
  }
  *Z = (Za + Zb) / 2;
...
#ifdef EVER_CALLED // Unused function, ad holten 12-2012
void intersect_3(double X1, double Y1, double Z1, double a1, double b1,
                 double c1, double X2, double Y2, double Z2, double a2,
                 double b2, double c2, double X3, double Y3, double Z3,
                 double a3, double b3, double c3, double *X, double *Y,
                 double *Z)
/* intersection, given three points with direction cosines */
{
  ...
  if (n_min > 2) /* min base in y-direction => take x */
    *Z = (X2 + X3 - 2 * X1 + 2 * Z1 * (a1 / c1) - Z2 * (a2 / c2) -
          Z3 * (a3 / c3)) /
         ((2 * a1 / c1) - (a2 / c2) - (a3 / c3));
  else /* min base in y-direction => take x */
    *Z = (Y2 + Y3 - 2 * Y1 + 2 * Z1 * (b1 / c1) - Z2 * (b2 / c2) -
          Z3 * (b3 / c3)) /
         ((2 * b1 / c1) - (b2 / c2) - (b3 / c3));

  *X = (X1 + (*Z - Z1) * (a1 / c1) + X2 + (*Z - Z2) * (a2 / c2) + X3 +
        (*Z - Z3) * (a3 / c3)) /
       3;
  *Y = (Y1 + (*Z - Z1) * (b1 / c1) + Y2 + (*Z - Z2) * (b2 / c2) + Y3 +
        (*Z - Z3) * (b3 / c3)) /
       3;
}
#endif
