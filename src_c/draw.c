...
Routines contained: 	drawcross, drawvector, draw_pnr, mark_detections
                                                mark_correspondences, mark_corr,
mark_track_c
...
Marker *markers[4]; /* lists with objects, like crosses, that are to be redrawn
                       after zooming */
...
void add_value(int x, int y, double value, int imgnr, char *color,
               int minzoomlevel) {
  Marker *pm;
  if (nmarkers[imgnr] >= maxnmarkers[imgnr])
    expand_markersbuffer(imgnr);
...
void add_vector(int imgnr, int x0, int y0, int x1, int y1, int width,
                char *color) {
  Marker *pm;
  if (nmarkers[imgnr] >= maxnmarkers[imgnr])
    expand_markersbuffer(imgnr);
...
void draw_objects(Tcl_Interp *interp, int imgnr) {
  ...
  for (i = 0; i < nmarkers[imgnr]; i++) {
    ...
    case OBJ_VALUE:
      ...
      break;
    case OBJ_VECTOR:
      drawvector(interp, pm->x, pm->y, pm->x1, pm->y1, pm->size, imgnr,
                 pm->color);
      break;
    }
  }
...
int drawmarker(Tcl_Interp *interp, Marker mark, int imgnr) {
  ...
  img_to_view_coordinates(&mark.x, &mark.y, (double)mark.x, (double)mark.y,
                          imgnr);
  sprintf(val, "mark .cam%d.pic %d %d %d", imgnr + 1, mark.x, mark.y,
          imgnr + 1);
  Tcl_Eval(interp, val);
  return TCL_OK;
}

int drawcross(Tcl_Interp *interp, int x0, int y0, int size, int imgnr,
              char color[]) {
  char val[256];
...
int draw_pnr(Tcl_Interp *interp, int x, int y, int pnr, int imgnr,
             char color[]) {
...
int draw_value(Tcl_Interp *interp, int x, int y, double pnr, int imgnr,
               char color[]) {
    ...
    for (i_img = 0; i_img < n_img; i_img++) {
      for (h = 0; h < nt4[3][i_img]; h++) {
        ...
        if (isinview(t4[3][i_img][h].x, t4[3][i_img][h].y, i_img)) {
          // replaced the next conversion by img_to_view_coordinates(), ad
          // holten, 04-2013 intx =
...
int trajectories_c(ClientData clientData, Tcl_Interp *interp, int argc,
                   const char **argv)
/* draws crosses for detected points in a displayed image */
{
  ...
  for (i = seq_first; i < seq_last; i++) {
    ...
    for (j = 0; j < anz1; j++) {
      ...
      if (m >= 0) {
        for (k = 0; k < n_img; k++) {
          img_coord(line1[j].x1, line1[j].y1, line1[j].z1, Ex[k], I[k], G[k],
                    ap[k], mmp, &p1[k].x, &p1[k].y);
          metric_to_pixel(p1[k].x, p1[k].y, imx, imy, pix_x, pix_y, &p1[k].x,
                          &p1[k].y, chfield);

          img_coord(line2[m].x1, line2[m].y1, line2[m].z1, Ex[k], I[k], G[k],
                    ap[k], mmp, &p2[k].x, &p2[k].y);
          metric_to_pixel(p2[k].x, p2[k].y, imx, imy, pix_x, pix_y, &p2[k].x,
                          &p2[k].y, chfield);
          ...
          if (isinview(p2[k].x, p2[k].y, k)) {
            ...
            img_to_view_coordinates(&intx1, &inty1, (int)p1[k].x, (int)p1[k].y,
                                    k);
            img_to_view_coordinates(&intx2, &inty2, (int)p2[k].x, (int)p2[k].y,
                                    k);
  ...
  if (line2 != NULL)
    free(line2); // ie. call free() if not deallocated yet
