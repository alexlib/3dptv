...
int epi_mm(double x1, double y1, Exterior Ex1, Interior I1, Glass G1,
           Exterior Ex2, Interior I2, Glass G2, mm_np mmp, double *xmin,
...
// double	*xmin, *ymin, *xmax, *ymax		output search window
{
  ...
  // ray_tracing	 (x1,y1, Ex1, I1,	  mmp, &X1, &Y1, &Z1, &a, &b,
  // &c);
...
void find_candidate(coord_2d crd[], target pix[], int num, double xa, double ya,
                    double xb, double yb, double eps, int n, int nx, int ny,
                    int sumg, candidate cand[], int *count, int nr)
...
// target	pix[]	pixel data for correlation
{
  ...
  m = (yb - ya) / (xb - xa);
  b = ya - m * xa; /* line equation: y = m*x + b */
