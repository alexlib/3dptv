...
Routines contained: 	pix_in_next, candsearch_in_pix, searchposition,
                                                predict, readseqtrackcrit,
readtrackdata, searchquader
...
int pix_in_next(target next[], int num, double x, double y, double dl,
                double dr, double du, double dd, int found[POSI]) {
  ...
  for (j = j0; j < num; j++) /* candidate search */
  {
    if (next[j].y > ymax)
      break; /* finish search */
    if (next[j].x > xmin && next[j].x < xmax && next[j].y > ymin &&
...
int candsearch_in_pix(target next[], int num, double x, double y, double dl,
                      double dr, double du, double dd, int p[4]) {
...
void searchquader(double X, double Y, double Z, double xr[4], double xl[4],
                  double yd[4], double yu[4]) {
  int k, i;
  double x, y, xz, yz;
  ...
  quader[3].z = Z + tpar.dvzmax; /* --+ */ // changed by Beat and JuliAn Nov
                                           // 2008
  ...
  for (i = 0; i < n_img; i++) {
    ...
    yd[i] = 0;
    yu[i] = imy;
    img_coord(point.x, point.y, point.z, Ex[i], I[i], G[i], ap[i], mmp, &xz,
              &yz);
    metric_to_pixel(xz, yz, imx, imy, pix_x, pix_y, &xz, &yz, chfield);

    for (k = 0; k < 8; k++) {
      img_coord(quader[k].x, quader[k].y, quader[k].z, Ex[i], I[i], G[i], ap[i],
                mmp, &x, &y);
