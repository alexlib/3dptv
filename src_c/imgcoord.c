...
void img_coord(double X, double Y, double Z, Exterior Ex, Interior I, Glass G,
  ...
  //	trans_Cam_Point(Ex, mm, G, X, Y, Z, &Ex_t, &X_t, &Y_t, &Z_t, &cross_p,
  //&cross_c);
  trans_Cam_Point(Ex, mm, G, X, Y, Z, &Ex_t, &X_t, &Y_t, &Z_t, cross_p,
                  cross_c);
  ...
  *x = I.xh -
       I.cc * (Ex.dm[0][0] * X + Ex.dm[1][0] * Y + Ex.dm[2][0] * Z) / deno;
  *y = I.yh -
       I.cc * (Ex.dm[0][1] * X + Ex.dm[1][1] * Y + Ex.dm[2][1] * Z) / deno;

  r = sqrt(*x * *x + *y * *y);

  dx = (*x) * (ap.k1 * r * r + ap.k2 * r * r * r * r +
               ap.k3 * r * r * r * r * r * r) +
       ap.p1 * (r * r + 2 * (*x) * (*x)) + 2 * ap.p2 * (*x) * (*y);
  dy = (*y) * (ap.k1 * r * r + ap.k2 * r * r * r * r +
               ap.k3 * r * r * r * r * r * r) +
...
void img_xy(double X, double Y, double Z, Exterior Ex, Interior I, Glass G,
  ...
  deno = Ex.dm[0][2] * (X - Ex.x0) + Ex.dm[1][2] * (Y - Ex.y0) +
         Ex.dm[2][2] * (Z - Ex.z0);

  *x = I.xh - I.cc *
                  (Ex.dm[0][0] * (X - Ex.x0) + Ex.dm[1][0] * (Y - Ex.y0) +
                   Ex.dm[2][0] * (Z - Ex.z0)) /
                  deno;

  *y = I.yh - I.cc *
                  (Ex.dm[0][1] * (X - Ex.x0) + Ex.dm[1][1] * (Y - Ex.y0) +
                   Ex.dm[2][1] * (Z - Ex.z0)) /
                  deno;
}
#endif
...
void img_xy_mm_geo(double X, double Y, double Z, Exterior Ex, Interior I,
  ...
  double deno;
  Exterior Ex_t;
  double X_t, Y_t, Z_t, cross_p[3],
      cross_c[3]; // removed Xh, Yh, Zh;	ad holten, 12-2012
