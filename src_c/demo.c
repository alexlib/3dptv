...
int flow_demo_c(ClientData clientData, Tcl_Interp *interp, int argc,
  ...
  for (i_seq = seq_first; i_seq <= seq_last; i_seq++) {
    ...
    if (pft_version == 4) {
      ...
      if (display) {
        for (j = 0; j < num[nr]; j++)
          drawcross(interp, (int)pix[nr][j].x, (int)pix[nr][j].y, cr_sz, nr,
                    "blue");
      ...
      if (fp1) {
        ...
        for (j = 0; j < num_points; j++) {
          ...
          //&geo[2][j].pnr);
          //		}
          //		if (n_img==2){ // Alex's patch. 24.09.09. Working on
          //Wesleyan data of 2 cameras only 			fscanf(fp1, "%d %lf %lf %lf %d %d %d
          //%d\n", 				&dumy, &fix[j].x, &fix[j].y, &fix[j].z, 				&geo[0][j].pnr,
          //&geo[1][j].pnr);
          //		}
          // by:
          for (i = 0; i < 4; i++)
        ...
        if (display) {
          for (j = 0; j < num_points; j++) {
            img_coord(fix[j].x, fix[j].y, fix[j].z, Ex[nr], I[nr], G[nr],
                      ap[nr], mmp, &pix[nr][j].x, &pix[nr][j].y);
            metric_to_pixel(pix[nr][j].x, pix[nr][j].y, imx, imy, pix_x, pix_y,
                            &pix[nr][j].x, &pix[nr][j].y, chfield);
            // if(geo[nr][j].pnr>-1){
            //	  drawcross (interp, (int) pix[nr][geo[nr][j].pnr].x, (int)
            //pix[nr][geo[nr][j].pnr].y,cr_sz, nr, "yellow");
            //}
            drawcross(interp, (int)pix[nr][j].x, (int)pix[nr][j].y, cr_sz, nr,
                      "red");
          }
