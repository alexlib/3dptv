...
int vrmltracks_c(ClientData clientData, Tcl_Interp *interp, int argc,
  ...
  }\n", i+1); fprintf(fp2, "   Transform {\n"); fprintf(fp2, "    translation
  %7.3f %7.3f %7.3f\n", Ex[i].x0, Ex[i].y0, Ex[i].z0); fprintf(fp2, " rotation 0
  0 1 3.14158 }\n");

          fprintf(fp2, "   MatrixTransform { matrix\n");
          for (k=0; k<3; k++)
  { fprintf(fp2, "    %7.3f %7.3f %7.3f 0\n", Ex[i].dm[k][0], Ex[i].dm[k][1],
  Ex[i].dm[k][2]); } fprintf(fp2, "       0 0 0 1 }\n"); fprintf(fp2, " Material
  {\n"); fprintf(fp2, "    ambientColor 0.25 0.25 0.25\n"); fprintf(fp2, "
  diffuseColor 1.00 0.%d 0.00 }\n", 2*(i+1)); fprintf(fp2, "   Cube { width
  %4.2f height %4.2f depth 2.5 } \n\n", imx*pix_x*5, imy*pix_y*5); fprintf(fp2,
  "   Material {\n"); fprintf(fp2, "    ambientColor 0.25 0.25 0.25\n");
    ...
    for (j = 0; j < anz1; j++) {
      ...
      if (m >= 0) {
        ...
        */
        ...
        fprintf(fp2, "    Separator {\n");
        fprintf(fp2, "    Transform {translation %7.3f %7.3f %7.3f}\n",mx, my,
        mz); fprintf(fp2, "    Transform {rotation 0 0 1 %7.5f}\n",rotz);
...
int vrmldetections_c(ClientData clientData, Tcl_Interp *interp, int argc,
  ...
  %7.3f %7.3f %7.3f\n", Ex[i].x0, Ex[i].y0, Ex[i].z0); fprintf(fp2, " rotation 0
  0 1 3.1416 }\n"); fprintf(fp2, "   MatrixTransform { matrix\n"); for (k=0;
  k<3; k++) { fprintf(fp2, "    %7.3f %7.3f %7.3f 0\n", Ex[i].dm[k][0],
  Ex[i].dm[k][1], Ex[i].dm[k][2]); } fprintf(fp2, "       0 0 0 1 }\n");
          fprintf(fp2, "   Material {\n");
          fprintf(fp2, "    ambientColor 0.25 0.25 0.25\n");
