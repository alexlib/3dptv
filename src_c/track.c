...
int trackcorr_c(ClientData clientData, Tcl_Interp *interp, int argc,
                const char **argv) {
  ...
  volumedimension(&X_lay[1], &X_lay[0], &Ymax, &Ymin, &Zmax_lay[1],
                  &Zmin_lay[0]);
  ...
  for (step = seq_first; step < seq_last; step++) {
    ...
    for (h = 0; h < m[1]; h++) {
      X1 = Y1 = Z1 = X0 = Y0 = Z0 = X2 = Y2 = Z2 = X5 = Y5 = Z5 = X3 = Y3 = Z3 =
          X4 = Y4 = Z4 = X6 = Y6 = Z6 = -999;
      mega[1][h].inlist = 0;
      ...
      */
      ...
      for (mm = 0; mm < zaehler1; mm++) /* zaehler1-loop */
      {
        ...
        for (i = 0; i < zaehler2; i++) {
          ...
          for (j = 0; j < n_img; j++)
            wn[i].whichcam[j] = p16[i].whichcam[j];
        }
        ...
        for (kk = 0; kk < zaehler2; kk++) { /* zaehler2-loop */
          ...
          angle = 2 * tpar.dangle;
          acc0 = 2 * tpar.dacc;
          angle0 = 2 * tpar.dangle;
          acc1 = 2 * tpar.dacc;
          angle1 = 2 * tpar.dangle;
          ...
          if (tpar.dvxmin < (X4 - X3) && (X4 - X3) < tpar.dvxmax &&
            ...
            if (okay == 1) {
              dl = (sqrt((X1 - X3) * (X1 - X3) + (Y1 - Y3) * (Y1 - Y3) +
                         (Z1 - Z3) * (Z1 - Z3)) +
              ...
              if ((acc < tpar.dacc && angle < tpar.dangle) ||
                ...
                mega[1][h].inlist++;
                /*
                     printf("alt mit kk h: %d, X3: %6.3f %6.3f %6.3f, dl: %5.3f,
                   ac:  %5.3f, an: %5.3f, quali: %d, rr: %5.3f\n", h, X3, Y3,
                   Z3, dl, acc, angle, quali, rr);
                */
              }
              okay = 0;
            }
        ...
        for (j = 0; j < n_img; j++) {
          if (x2[j] != -1e10 && y2[j] != -1e10) {
            pixel_to_metric(x2[j], y2[j], imx, imy, pix_x, pix_y, &x2[j],
                            &y2[j], chfield);
            quali++;
          }
        }
        ...
        if (quali >= 2) {
          ...
          det_lsq_3d(Ex, I, G, ap, mmp, x2[0], y2[0], x2[1], y2[1], x2[2],
                     y2[2], x2[3], y2[3], &X4, &Y4, &Z4);

          /* volume check */
          if (X_lay[0] < X4 && X4 < X_lay[1] && Ymin < Y4 && Y4 < Ymax &&
              Zmin_lay[0] < Z4 && Z4 < Zmax_lay[1]) {
            invol = 1;
          }

          /* displacement check */
          if (invol == 1 && tpar.dvxmin < (X3 - X4) &&
              (X3 - X4) < tpar.dvxmax && tpar.dvymin < (Y3 - Y4) &&
              (Y3 - Y4) < tpar.dvymax && tpar.dvzmin < (Z3 - Z4) &&
              (Z3 - Z4) < tpar.dvzmax) {
            okay = 1;
            if (okay == 1) {
              rr = 1000000;
              ...
              if ((acc < tpar.dacc && angle < tpar.dangle) ||
                ...
                rr = (dl / lmax + acc / tpar.dacc + angle / tpar.dangle) /
                     (quali + w[mm].freq);
                /*
                     printf("neu ohne prev h: %d, X3: %6.3f %6.3f %6.3f, dl:
                   %5.3f, ac:	%5.3f, an: %5.3f, quali: %d, rr: %5.3f\n", h,
        ...
        if (mega[1][h].inlist == 0 && mega[1][h].prev >= 0) {
          ...
          if (tpar.dvxmin < (X3 - X1) && (X3 - X1) < tpar.dvxmax &&
            ...
            if (okay == 1) {
              ...
              dl = (sqrt((X1 - X0) * (X1 - X0) + (Y1 - Y0) * (Y1 - Y0) +
                         (Z1 - Z0) * (Z1 - Z0)) +
              ...
              if ((acc < tpar.dacc && angle < tpar.dangle) ||
                ...
                mega[1][h].inlist++;
                /*
               printf("alt ohne kk h: %d, X3: %6.3f %6.3f %6.3f, dl: %5.3f, ac:
               %5.3f, an: %5.3f, quali: %d, rr: %5.3f\n", h, X3, Y3, Z3, dl,
               acc, angle, quali, rr);
                */
              }
            }
      ...
      if (tpar.add) {
        if (mega[1][h].inlist == 0 && mega[1][h].prev >= 0) {
          /*
            ...
            mega[1][h].prev); printf("X1: %6.3f  %6.3f	%6.3f, h: %d\n", X1, Y1,
            Z1, h); printf("X2: %6.3f  %6.3f	%6.3f\n", X2, Y2, Z2);
          ...
          */
          ...
          if (quali >= 2) {
            ...
            /* displacement check */
            if (invol == 1 && tpar.dvxmin < (X2 - X3) &&
                (X2 - X3) < tpar.dvxmax && tpar.dvymin < (Y2 - Y3) &&
                (Y2 - Y3) < tpar.dvymax && tpar.dvzmin < (Z2 - Z3) &&
                (Z2 - Z3) < tpar.dvzmax) {
              okay = 1;
              if (okay == 1) {
                rr = 1000000;
    ...
    for (h = 0; h < m[1]; h++) {
      if (mega[1][h].inlist > 0) {
        ...
        /* best choice was already used by mega[2][mega[1][h].next].prev */
          ...
          if (mega[1][mega[2][mega[1][h].next].prev].finaldecis >
              mega[1][h].finaldecis) {
            /*
                    printf("h ist besser, h: %4d jetzt: %5.3f vorher: %d
               %5.3f\n", h, mega[1][h].finaldecis,
               mega[1][mega[2][mega[1][h].next].prev].next,
            ...
            */
            ...
            /*
                    printf("h ist schlechter, h: %4d jetzt: %5.3f vorher: %d
               %5.3f\n", h, mega[1][h].finaldecis,
               mega[1][mega[2][mega[1][h].next].prev].next,
                                    mega[1][mega[2][mega[1][h].next].prev].finaldecis);
            */
            ...
            if (c4[1][h].p[j] > 0 && c4[2][mega[1][h].next].p[j] > 0) {
              ...
              if (isinview(xp[j], yp[j], j)) {
                ...
                if (mega[1][h].finaldecis > 0.2) {
                  draw_pnr_autohide(interp, intx0, inty0, h, j, "white", 2,
                                    zoompar[i].fac);
                  draw_pnr_autohide(interp, intx0, inty0 + 10, mega[1][h].next,
                                    j, val, 2, zoompar[i].fac);
                  draw_value_autohide(interp, intx0, inty0 + 20,
                                      mega[1][h].finaldecis, j, val, 2,
                                      zoompar[i].fac);
                }
              }
            }
...
int trackback_c(ClientData clientData, Tcl_Interp *interp, int argc,
                const char **argv) {
  char buf[256];
  ...
  for (step = seq_last - 1; step > seq_first; step--) {
    ...
    for (h = 0; h < m[1]; h++) {
      if (mega[1][h].next >= 0 && mega[1][h].prev == -1) {
        X1 = Y1 = Z1 = X0 = Y0 = Z0 = X2 = Y2 = Z2 = X5 = Y5 = Z5 = X3 = Y3 =
            Z3 = X4 = Y4 = Z4 = X6 = Y6 = Z6 = -999;
            ...
            if (tpar.dvxmin < (X1 - X3) && (X1 - X3) < tpar.dvxmax &&
              ...
              if (okay == 1) {
                ...
                if ((acc < tpar.dacc && angle < tpar.dangle) ||
                  ...
                  mega[1][h].inlist++;
                  /*
                  printf("h: %d, old and ok: X3: %6.3f %6.3f %6.3f, ftnr: %d,
                  prev: %d, next: %d, angle: %6.3f, acc: %6.3f, dl: %6.3f, rr:
                  %6.3f, quali: %d\n", h, X3, Y3, Z3, w[i].ftnr,
                  mega[2][w[i].ftnr].prev, mega[2][w[i].ftnr].next,angle, acc,
                  dl, rr, quali);
                  */
                }
              }
        ...
        if (tpar.add) {
          if (mega[1][h].inlist == 0) {
            for (j = 0; j < n_img; j++) {
              /* use fix distance to define xl, xr, yu, yd instead of
               * searchquader */
              xl[j] = xr[j] = yu[j] = yd[j] = 3.0;
              // stripped & from philf, ad holten 12-2012
              zaehler1 =
            ...
            if (quali >= 2) {
              ...
              /* displacement check */
              if (invol == 1 && tpar.dvxmin < (X1 - X3) &&
                  (X1 - X3) < tpar.dvxmax && tpar.dvymin < (Y1 - Y3) &&
                  (Y1 - Y3) < tpar.dvymax && tpar.dvzmin < (Z1 - Z3) &&
                  (Z1 - Z3) < tpar.dvzmax) {
                okay = 1;
                if (okay == 1) {
                  dl = (sqrt((X1 - X0) * (X1 - X0) + (Y1 - Y0) * (Y1 - Y0) +
        ...
        } /* end of if old wasn't found try to create new particle position from
             rest */
      }
    } /* end of h-loop */
    ...
    for (h = 0; h < m[1]; h++) {
      if (mega[1][h].inlist > 0) {
        ...
        if (mega[2][mega[1][h].linkdecis[0]].prev == -1 &&
            mega[2][mega[1][h].linkdecis[0]].next == -2) {
          mega[1][h].finaldecis = mega[1][h].decis[0];
        ...
        if (mega[2][mega[1][h].linkdecis[0]].prev != -1 &&
          ...
          printf("check old, new: X3: %6.3f, Y3: %6.3f, Z3: %6.3f, angle: %6.3f,
          acc: %6.3f\n", X3, Y3, Z3, angle, acc);
          */
          if ((acc < tpar.dacc && angle < tpar.dangle) ||
              (acc < tpar.dacc / 10)) {
