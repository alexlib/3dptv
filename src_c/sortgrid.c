...
Description:		reads objects, detected by detection etc.,
                                        sorts them with respect to the 13 wires
of the grid, detects missing points and writes the result to  a new file
...
void just_plot(Tcl_Interp *interp, Exterior Ex, Interior I, Glass G, ap_52 ap,
               ...
               coord_3d fix[], int field, int n_img) {
  int i, intx, inty;
...
void sortgrid_man(Tcl_Interp *interp, Exterior Ex, Interior I, Glass G,
                  ap_52 ap, mm_np mm, int imx, int imy, double pix_x,
                  double pix_y, int nfix, coord_3d fix[], int num, target pix[],
                  int field, int n_img) {
  int i, j, intx, inty;
  double xp, yp, eps = 10.0;
  //	target	old[512];
...
void sortgrid_file(Tcl_Interp *interp, Exterior Ex, Interior I, Glass G,
                   ap_52 ap, mm_np mm, int imx, int imy, double pix_x,
                   double pix_y, int nfix, coord_3d fix[], int num,
                   target pix[], int field, int n_img) {
  int i, k;
  double eps = 10.0;
  target old[1024];
  char file_sort[256];
  ...
  if (!fpp)
    return;
  k = 0;
  while (fscanf(fpp, "%d %lf %lf %lf %d", &fix[k].pnr, &fix[k].x, &fix[k].y,
                &fix[k].z, &dummy) != EOF) {
