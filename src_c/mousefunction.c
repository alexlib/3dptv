...
int mouse_proc_c(ClientData clientData, Tcl_Interp *interp, int argc,
                 const char **argv)
                        ...
                        argc[4] : image number			*/
{
  ...
  case 2:
    ...
    for (i = 0; i < n_img; i++) {
      ...
      epi_mm(x, y, Ex[n], I[n], G[n], Ex[i], I[i], G[i], mmp, &xa12, &ya12,
             &xb12, &yb12);
      xa = (xa12 + xb12) / 2;
      ya = (ya12 + yb12) / 2;
  ...
  case 5: /* measure coordinates and grey value */
    ...
    add_mark(
        click_x, click_y,
        n); // mark is already drawn by tcl code, just add, ad holten 03-2013
    break;
  ...
  case 1:
    ...
    if (zoompar.fac != zoomfac || zoompar.fixed != fixed) {
      ...
      alpha =
          img_block.pixelPtr[3]; // the alpha value of the last image plotted
      ...
      sprintf(val, "newimage %d %f %f %d %d", n + 1, (double)click_x / imx,
              (double)click_y / imy, zoomfac, fixed);
      Tcl_Eval(interp, val);
  ...
  case 10:
    ...
    for (i = 0; i < n_img; i++) {
      ...
      if (zoompar.fac != zoomfac ||
          zoompar.fixed != fixed) { // we have to create a new image
        ...
        sprintf(val, "newimage %d %f %f %d %d", i + 1, zoompar.xc, zoompar.yc,
                zoomfac, fixed);
        Tcl_Eval(interp, val);
  ...
  case 3: /* generate epipolar line segments */
    ...
    // intx1 = (int) ( imx/2 + zoom_f[n] * (pix[n][pt1].x-zoom_x[n]));  changed,
    // ad holten inty1 = (int) ( imy/2 + zoom_f[n] * (pix[n][pt1].y-zoom_y[n]));
    ...
    for (i = 0; i < n_img; i++)
      if (i != n) {
        ...
        epi_mm(geo[n][k].x, geo[n][k].y, Ex[n], I[n], G[n], Ex[i], I[i], G[i],
               mmp, &xa12, &ya12, &xb12, &yb12);

        /* search candidate in img[i] */
        printf("\ncandidates in img: %d\n", i);
        find_candidate_plus_msg(
            geo[i], pix[i], num[i], xa12, ya12, xb12, yb12, eps0, pix[n][pt1].n,
            pix[n][pt1].nx, pix[n][pt1].ny, pix[n][pt1].sumg, cand, &count, i);
        ...
        xb12 += I[i].xh;
        yb12 += I[i].yh;
        metric_to_pixel(xa12, ya12, imx, imy, pix_x, pix_y, &xa12, &ya12,
                        chfield);
        metric_to_pixel(xb12, yb12, imx, imy, pix_x, pix_y, &xb12, &yb12,
                        chfield);
        ...
        for (j = 0; j < count; j++) {
          ...
          img_to_view_coordinates(&intx1, &inty1, pix[i][pt2].x, pix[i][pt2].y,
                                  n);
  ...
  case 4: /* delete points, which should not be used for orientation */
    ...
    if (j != -1) {
      num[n] -= 1;
      sprintf(buf, "point %d deleted", j);
      puts(buf);
    } else
      sprintf(buf, "no point near click coord !");
    puts(buf);
    ...
    //	if (zoom_x[n] < imx/(2*zoom_f[n]))	   zoom_x[n] =
    //imx/(2*zoom_f[n]); 	if (zoom_x[n] > imx-imx/(2*zoom_f[n])) zoom_x[n] =
    //imx-imx/(2*zoom_f[n]); 	if (zoom_y[n] < imy/(2*zoom_f[n]))	   zoom_y[n] =
    //imy/(2*zoom_f[n]); 	if (zoom_y[n] > imy-imy/(2*zoom_f[n])) zoom_y[n] =
    //imy-imy/(2*zoom_f[n]);
    ...
    //		if (zoom_x[0] < imx/(2*zoom_f[0]))	   zoom_x[0] =
    //imx/(2*zoom_f[0]); 		if (zoom_x[0] > imx-imx/(2*zoom_f[0])) zoom_x[0] =
    //imx-imx/(2*zoom_f[0]); 		if (zoom_y[0] < imy/(2*zoom_f[0]))	   zoom_y[0] =
    //imy/(2*zoom_f[0]); 		if (zoom_y[0] > imy-imy/(2*zoom_f[0])) zoom_y[0] =
    //imy-imy/(2*zoom_f[0]);
    ...
    //	}
    //	else {
    //		sprintf(val,"Zooming of corresp. areas is only possible by clicking in
    //image 1"); 		Tcl_SetVar(interp, "tbuf", val, TCL_GLOBAL_ONLY);
    //		Tcl_Eval(interp, ".text delete 2");
    //		Tcl_Eval(interp, ".text insert 2 $tbuf");
    //	}
