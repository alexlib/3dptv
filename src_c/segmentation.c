...
// int           nr;           image number for display
  ...
  if (examine == 3) {
    ...
    if (tiff_flag) {
      write_tiff(lp_name, img_lp, imx, imy);
    } else {
      fwrite(img_lp, 1, imgsize, fp);
    }
  ...
  case 0:
    break;
  case 1:
    lowpass_3(img_hp, img_hp);
    break;
  case 2:
    filter_3(img_hp, img_hp);
    break;
  }
  ...
  if (examine == 3) {
    fp = fopen(hp_name, "w");
    if (tiff_flag) {
      write_tiff(hp_name, img_hp, imx, imy);
    } else {
      fwrite(img_hp, 1, imgsize, fp);
    }
    fclose(fp);
  }
  free(img_lp);
...
*/
  ...
  for (i = ymin + 1; i < ymax - 1; i++)
    for (j = xmin + 1; j < xmax - 1; j++) {
      gv = *(img + i * imx + j);
      if (gv > thres && gv >= *(img + i * imx + j - 1) &&
          gv >= *(img + i * imx + j + 1) && gv >= *(img + (i - 1) * imx + j) &&
          gv >= *(img + (i + 1) * imx + j) &&
          gv >= *(img + (i - 1) * imx + j - 1) &&
          gv >= *(img + (i + 1) * imx + j - 1) &&
          gv >= *(img + (i - 1) * imx + j + 1) &&
...
/*
   ...
   set to zero char          par_file[]			name of parameter file
        int           xmin,xmax,ymin,ymax	search area
        target        pix[]					pixel coord
   array, global
        int           nr					image number for
   display int           *num					number of
   detections
*/
/*    thresholding and center of gravity with a peak fitting technique  */
/*    uses 4 neighbours for connectivity and 8 to find local maxima      */
        ...
        *(img + i * imx + j) = 0;
        ...
        while (n_wait > 0) {
          ...
          for (n = 0; n < 4; n++) {
            ...
            /* conditions for threshold, discontinuity, image borders */
            /* and peak fitting */
            if (gv > thres && xn >= xmin && xn < xmax && yn >= ymin &&
                yn < ymax) {
              sumg += gv;
              *(img + imx * yn + xn) = 0;
              if (xn < xa)
