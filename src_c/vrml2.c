...
int vrmltracks_c(ClientData clientData, Tcl_Interp *interp, int argc,
                 const char **argv) {
  int i, anz1, anz2, m, j;
  FILE *fp1, *fp2;
  char val[256];
  ...
  fprintf(fp2, "    appearance Appearance {material Material { diffuseColor 0 "
               "0 1 } } }\n");
  ...
  for (i = seq_first; i < seq_last; i++) {
    ...
    for (j = 0; j < anz1; j++) {
      ...
      if (m >= 0) {
        ...
        fprintf(fp2,
                "    Shape { geometry Box { size %3.2f %3.2f %3.2f }\n      "
                "appearance USE step%d }\n",
                cubes, cubes, cubes, i);

        fprintf(fp2,
                "    Shape { geometry Sphere { radius %3.2f }\n      "
                "appearance USE step%d }\n",
                cubes, i);
        ...
        fprintf(fp2,
                "    children [ Shape { geometry Cylinder { radius %3.2f "
                "height %3.2f } \n      appearance USE step%d } ]\n",
                cubes / 4, dl, i);
        fprintf(fp2,
                "    children [ Shape { geometry Box {size %3.2f %3.2f %3.2f } "
                "\n      appearance USE step%d } ]\n\n",
        ...
        fprintf(fp2, "    Shape { geometry IndexedLineSet {\n");
        fprintf(fp2, "     coord Coordinate { point [\n");
        fprintf(fp2, "      %7.3f %7.3f %7.3f,\n", line1[j].x1, line1[j].y1,
                line1[j].z1);
        fprintf(fp2, "      %7.3f %7.3f %7.3f, ] }\n", line2[m].x1, line2[m].y1,
                line2[m].z1);
        fprintf(
            fp2,
            "     coordIndex [ 0, 1, -1] } \n      appearance USE step%d}\n\n",
  ...
  Tcl_Eval(interp,
           ".text insert 2 \"Tracks written to VRML-File: tracks.wrl\"");
...
int vrmldetections_c(ClientData clientData, Tcl_Interp *interp, int argc,
                     const char **argv) {
  int i, anz1, j, dumy;
  FILE *fp1, *fp2;
  char val[256];
  ...
  for (i = seq_first; i <= seq_last; i++) {
    ...
    line1 = (vector *)calloc(anz1, sizeof(vector));
    for (j = 0; j < anz1; j++) {
      fscanf(fp1, "%d %lf %lf %lf %d %d %d %d\n", &line1[j].p, &line1[j].x1,
             &line1[j].y1, &line1[j].z1, &dumy, &dumy, &line1[j].type, &dumy);
    }
    fclose(fp1);
    ...
    for (j = 0; j < anz1; j++) {
      ...
      fprintf(fp2, "    children [\n");
      fprintf(fp2,
              "    Shape { geometry Box { size %3.2f %3.2f %3.2f } appearance "
              "USE step%d }\n",
              cubes, cubes, cubes, i);
      /*
      fprintf(fp2, "    Shape { geometry Sphere { radius %3.2f } appearance USE
      step%d }\n", cubes, i);
      */
      fprintf(fp2, "    ] }\n");
    }
  ...
  Tcl_Eval(
      interp,
      ".text insert 2 \"Detections written to VRML-File: detections.wrl\"");
  Tcl_Eval(interp, "update idletasks");
...
int vrmldettracks_c(ClientData clientData, Tcl_Interp *interp, int argc,
                    const char **argv) {
  int i, anz1, anz2, m, j;
  FILE *fp1, *fp2;
  char val[256];
  ...
  for (i = seq_first; i <= seq_last; i++) {
    if (i < 10)
      sprintf(val, "res/ptv_is.%1d", i);
    else if (i < 100)
      sprintf(val, "res/ptv_is.%2d", i);
    else
      sprintf(val, "res/ptv_is.%3d", i);
    ...
    if (i < seq_last) {
      /* read next time step */
      if (i + 1 < 10)
        sprintf(val, "res/ptv_is.%1d", i + 1);
      else if (i + 1 < 100)
        sprintf(val, "res/ptv_is.%2d", i + 1);
      else
        sprintf(val, "res/ptv_is.%3d", i + 1);
      ...
      */
      ...
      if (i < seq_last) {
        ...
        if (m >= 0) {
          ...
          fprintf(fp2, "    Transform { translation %7.3f %7.3f %7.3f\n",mx, my,
          mz); fprintf(fp2, "    children [ Transform {rotation 0 0 1
          %7.5f\n",rotz); fprintf(fp2, "    children [ Transform {rotation 1 0 0
          %7.5f \n",rotx); fprintf(fp2, "    children [ Shape { geometry
          Cylinder { radius %3.2f height %3.2f } \n      appearance USE step%d }
          ]\n",cubes/4, dl, i); fprintf(fp2, "    children [ Shape { geometry
          Box {size %3.2f %3.2f %3.2f } \n      appearance USE step%d }
  ...
  if (line1)
    free(line1); // added, ad holten 12-2012
  if (line1)
    free(line1);
  fprintf(fp2, "# trajectories finished\n");
  ...
  Tcl_Eval(interp, ".text delete 2");
  Tcl_Eval(interp,
           ".text insert 2 \"Tracks/Detections written to VRML-File: dt.wrl\"");
  Tcl_Eval(interp, "update idletasks");
