...
static int FileMatchTIF(Tcl_Channel chan, CONST char *fileName,
                        Tcl_Obj *formatString, int *widthPtr, int *heightPtr,
                        Tcl_Interp *interp);

static int FileReadTIF(Tcl_Interp *interp, Tcl_Channel chan,
                       CONST char *fileName, Tcl_Obj *format,
                       Tk_PhotoHandle imageHandle, int destX, int destY,
                       int width, int height, int srcX, int srcY);

static int FileWriteTIF(Tcl_Interp *interp, CONST char *fileName,
...
void tkTIFFErrorHandler(const char *module, const char *fmt, va_list ap) {
  if (module != NULL)
...
int FileMatchTIF(Tcl_Channel chan, CONST char *fileName, Tcl_Obj *formatString,
...
int FileReadTIF(Tcl_Interp *interp, Tcl_Channel chan, CONST char *fileName,
                Tcl_Obj *format, Tk_PhotoHandle imageHandle, int destX,
  ...
  if (tif == NULL) {
    ...
    Tcl_AppendResult(interp, "couldn't read TIF header from file \"", fileName,
                     "\"", NULL);
    TIFFSetErrorHandler(prev_handler);
    return TCL_ERROR;
  }
  ...
  */
  ...
  if ((width <= 0) || (height <= 0) || (srcX >= (int)fileImgWidth) ||
      (srcY >= (int)fileImgHeight)) {
    return TCL_OK;
  }
  ...
  */
  ...
  for (h = height; h > 0; h -= nLines) {
    ...
    for (row = startrow; row < (startrow + nLines); row++) {
      ...
      pixelPtrIndex =
          row * block.pitch - (n - 1) * nDefBlockLines * block.pitch;
      /* test entry !!! */
    ...
    Tk_PhotoPutBlock(interp, imageHandle, &block, destX, destY, width, nLines,
...
int FileWriteTIF(interp, fileName, formatString, blockPtr) Tcl_Interp *interp;
CONST char *fileName;
