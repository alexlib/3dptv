...
void checkpoint_proc(Tcl_Interp *interp) {
  ...
  for (i = 0; i < nfix; i++) {
    ...
    /* safety check */
    if (n_img > 0 && crd[0][i].pnr != fix[i].pnr)
      continue;
    if (n_img > 1 && crd[1][i].pnr != fix[i].pnr)
      continue;
    if (n_img > 2 && crd[2][i].pnr != fix[i].pnr)
      continue;
    if (n_img > 3 && crd[3][i].pnr != fix[i].pnr)
      continue;
    ...
    switch (n_img) {
    case 2:
      det_lsq_2(Ex, I, G, ap, mmp, crd[0][i].x, crd[0][i].y, crd[1][i].x,
                crd[1][i].y, &X, &Y, &Z);
      break;
    case 3:
      det_lsq_3(Ex, I, G, ap, mmp, crd[0][i].x, crd[0][i].y, crd[1][i].x,
                crd[1][i].y, crd[2][i].x, crd[2][i].y, &X, &Y, &Z);
      break;
    case 4:
      det_lsq_4(Ex, I, G, ap, mmp, crd[0][i].x, crd[0][i].y, crd[1][i].x,
                crd[1][i].y, crd[2][i].x, crd[2][i].y, crd[3][i].x, crd[3][i].y,
                &X, &Y, &Z);
      break;
    }
  ...
  printf("a posteriori	 => 			   RMS = %6.3f/%6.3f/%6.3f "
         "mm\n",
