...

{
  ...
  case 1:
    ...
    if (zoom_x[n] < imx / (2 * zoom_f[n]))
      zoom_x[n] = imx / (2 * zoom_f[n]);
    if (zoom_x[n] > imx - imx / (2 * zoom_f[n]))
      zoom_x[n] = imx - imx / (2 * zoom_f[n]);
    if (zoom_y[n] < imy / (2 * zoom_f[n]))
      zoom_y[n] = imy / (2 * zoom_f[n]);
    if (zoom_y[n] > imy - imy / (2 * zoom_f[n]))
      zoom_y[n] = imy - imy / (2 * zoom_f[n]);
  ...
  case 2: /* zoom images at corresponding position by factor zf */
    ...
    if (n == 0) {
      ...
      if (zoom_x[0] < imx / (2 * zoom_f[0]))
        zoom_x[0] = imx / (2 * zoom_f[0]);
      if (zoom_x[0] > imx - imx / (2 * zoom_f[0]))
        zoom_x[0] = imx - imx / (2 * zoom_f[0]);
      if (zoom_y[0] < imy / (2 * zoom_f[0]))
        zoom_y[0] = imy / (2 * zoom_f[0]);
      if (zoom_y[0] > imy - imy / (2 * zoom_f[0]))
        zoom_y[0] = imy - imy / (2 * zoom_f[0]);
      ...
      for (i = 1; i < n_img; i++) {
        ...
        epi_mm(x, y, Ex[0], I[0], Ex[i], I[i], mmp, &xa12, &ya12, &xb12, &yb12);
        xa = (xa12 + xb12) / 2;
        ya = (ya12 + yb12) / 2;
      ...
      sprintf(
          val,
          "Zooming of corresp. areas is only possible by clicking in image 1");
  ...
  case 3: /* generate epipolar line segments */
    ...
    for (i = 0; i < n_img; i++)
      if (i != n) {
        ...
        epi_mm(geo[n][k].x, geo[n][k].y, Ex[n], I[n], Ex[i], I[i], mmp, &xa12,
               &ya12, &xb12, &yb12);
  ...
  case 5: /* measure coordinates and grey value */
    ...
    sprintf(buf, "	%6.2f	 %6.2f	  %s", x, y, argv[5]);
    puts(buf);
    Tcl_SetVar(interp, "tbuf", buf, TCL_GLOBAL_ONLY);
    Tcl_Eval(interp, ".text delete 2");
    Tcl_Eval(interp, ".text insert 2 $tbuf");
